...
9a. **Check for an open Dependabot pull request against the action pins.** Every
    ...
    workflow runs with `contents: write` and `packages: write`. Dependabot does
    maintain SHA pins and rewrites the trailing version comment, so review its
    PR rather than re-pinning by hand. Major bumps in that PR are not
    automatically safe: they are why the pins were set to the versions in use
